...
int i2c_write(uint8_t slave_addr, uint8_t reg_addr, uint8_t data_len, uint8_t const *data)
{
    ...
    bool i2c_write_done = false;
    i2c_master_write_packet_wait(&i2c_master_mpu9250_instance, &i2c_wpacket);
...
int i2c_read(uint8_t slave_addr, uint8_t reg_addr, uint8_t data_len, uint8_t *data)
{
    ...
    i2c_master_write_packet_wait_no_stop(&i2c_master_mpu9250_instance, &i2c_wpacket);
    while(!i2c_write_done){
        timeout++;
        if(timeout >= I2C_TIMEOUT_CNT) {
...
{
    uint32_t load = (unsigned long *)(26000 - dualtimer_get_value(DUALTIMER_TIMER2));
    count = load / 26000;
}
#define log_i       DBG_LOG
#define log_e       DBG_LOG_DEV
#define delay_ms    UNUSED
