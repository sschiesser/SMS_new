...
#define GYRO_MEAS_ERROR			(PI * (40.0 / 180.0))
#define GYRO_MEAS_DRIFT			(PI * (0.0 / 180.0))
...
#define BETA				(sqrt(3.0 / 4.0) * GYRO_MEAS_ERROR)
#define ZETA				(sqrt(3.0 / 4.0) * GYRO_MEAS_DRIFT)
...
void ahrs_calculation(float *q) {
	//mpu_device.output.yaw = atan2(2.0 * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]);
	//mpu_device.output.pitch = -asin(2.0f * (q[1] * q[3] - q[0] * q[2]));
