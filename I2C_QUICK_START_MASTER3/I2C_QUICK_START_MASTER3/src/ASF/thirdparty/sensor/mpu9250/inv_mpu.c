...
*  @param[in]  mpu_reg     Register address.
*  @param[out] data    Register data.
*  @return     0 if successful.
*/
int mpu_read_reg(unsigned char mpu_reg, unsigned char *data)
{
    if (mpu_reg == st.reg->fifo_r_w || mpu_reg == st.reg->mem_r_w)
    return -1;
    if (mpu_reg >= st.hw->num_reg)
    return -1;
    return i2c_read(st.hw->addr, mpu_reg, 1, data);
}
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (i2c_read(st.hw->addr, REG_6500_XA_ST_DATA, 3, regs)) {
        if(debug)
        DBG_LOG("Reading OTP Register Error.\n");
        return 0x07;
    }
    if(debug)
    DBG_LOG("Accel OTP:%d, %d, %d\n", regs[0], regs[1], regs[2]);
    for (i = 0; i < 3; i++) {
        if (regs[i] != 0) {
    ...
    if(result == 0) {
        ...
        if(debug)
        DBG_LOG("Accel:CRITERIA C: bias less than %7.4f\n", accel_offset_max/1.f);
        for (i = 0; i < 3; i++) {
            if(fabs(bias_regular[i]) > accel_offset_max) {
                if(debug)
                DBG_LOG("FAILED: Accel axis:%d = %ld > 500mg\n", i, bias_regular[i]);
                result |= 1 << i;	//Error condition
            }
        }
    }
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if(otp_value_zero == 0) {
        ...
        if(debug) {
            DBG_LOG("GYRO:CRITERIA B\r\n");
            DBG_LOG("Max DPS: %7.4f\r\n", gyro_st_al_max/1.f);
        }
        ...
        for (i = 0; i < 3; i++) {
            ...
            if(st_shift_cust[i] < gyro_st_al_max) {
                if(debug)
                DBG_LOG("GYRO FAIL axis:%d greater than 60dps\n", i);
                result |= 1 << i;	//Error condition
            }
        }
    }
    ...
    if(result == 0) {
        ...
        if(debug)
        DBG_LOG("Gyro:CRITERIA C: bias less than %7.4f\n", gyro_offset_max/1.f);
        for (i = 0; i < 3; i++) {
            if(fabs(bias_regular[i]) > gyro_offset_max) {
                if(debug)
                DBG_LOG("FAILED: Gyro axis:%d = %ld > 20dps\n", i, bias_regular[i]);
                result |= 1 << i;	//Error condition
            }
        }
    }
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    if(debug)
    DBG_LOG("Starting Bias Loop Reads\n");
...
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
    ...
    if (ii == tries) {
        ...
        */
        if(debug)
        DBG_LOG("Retrieving Biases Error - possible I2C error\n");
    ...
    if (ii == tries) {
        ...
        if(debug)
        DBG_LOG("Retrieving ST Biases Error - possible I2C error\n");
    ...
    if(debug)
    DBG_LOG("Accel Self Test Results: %d\n", accel_result);

    gyro_result = gyro_6500_self_test(gyro, gyro_st, debug);
    if(debug)
    DBG_LOG("Gyro Self Test Results: %d\n", gyro_result);
    ...
    if(debug)
    DBG_LOG("Compass Self Test Results: %d\n", compass_result);
    if (!compass_result)
    result |= 0x04;
    #else
    result |= 0x04;
